...
RUN --mount=type=cache,id=s/db32d8da-8fa3-4951-b3c3-06b91af0f6ef-apt-cache,target=/var/cache/apt,sharing=locked \
  --mount=type=cache,id=s/db32d8da-8fa3-4951-b3c3-06b91af0f6ef-apt-lists,target=/var/lib/apt/lists,sharing=locked \
  apt-get update \
  && apt-get upgrade -y \
...
# broken image.
RUN --mount=type=cache,id=s/db32d8da-8fa3-4951-b3c3-06b91af0f6ef-npm-cache,target=/root/.npm \
...
COPY . ./
RUN --mount=type=cache,id=s/db32d8da-8fa3-4951-b3c3-06b91af0f6ef-next-cache,target=/app/.build/next/cache \
  mkdir -p /app/data \
  && npm run build \
